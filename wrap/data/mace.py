...
class XYZ4MACEMP(XYZ):
    ...
    def _extract_properties(self, mol, i, atomic_energies, dtype=torch.float32):     
        ...
        return Data(edge_index=torch.tensor(edge_index, dtype=torch.long),
                    ## TODO should node_attrs be long?
...
class XYZLarge4MACEMP(XYZLarge):
    """Data class for MACE models processed from extxyz files.
    ...
    def _extract_properties(self, mol, i, refs):
        ...
        if 'forces' in mol.arrays.keys():
            forces = mol.get_forces()
        else:
            forces = np.zeros_like(mol.get_positions())
            


        return Data(edge_index=torch.tensor(edge_index, dtype=torch.long),
                    node_attrs=one_hot.type(torch.float32),
                    positions=torch.Tensor(mol.get_positions()).type(torch.float32),
                    shifts=torch.Tensor(shifts).type(torch.float32),
                    unit_shifts=torch.Tensor(unit_shifts).type(torch.float32),
                    cell=torch.Tensor(np.array(mol.cell)).type(torch.float32),
                    forces=torch.Tensor(forces).type(torch.float32),
                    energy=torch.Tensor([energy]).type(torch.float32),
                    reference_energy=torch.Tensor([reference_energy]).type(torch.float32),
                    n_atoms=torch.Tensor([len(mol)]).type(torch.int),
                    idx=torch.IntTensor([i]),
                )

class MACEAtomsConverter:
    """A class to convert atoms objects for the mace model."""

    def __init__(self, mace_model, total_energy = True, statistics_file: str = None):
        """Initialize self given the mace model in the arguments."""
        self.model = mace_model
        self.z_table = utils.AtomicNumberTable([int(z) for z in self.model.models[0].atomic_numbers])
        self.r_max = self.model.models[0].r_max.item()
        self.refs = read_stats(statistics_file) if statistics_file is not None else {"atomic_energies": [0.] * len(atomic_numbers)}
        self.total_energy = total_energy

    def _extract_properties(self, ats):
        ...
        indices = atomic_numbers_to_indices(ats.get_atomic_numbers(), z_table=self.z_table)
        one_hot = to_one_hot(
        ...
        # # reference against atom energies
        if not self.total_energy:
            reference_energy = sum([self.refs['atomic_energies'][elem] for elem in ats.get_atomic_numbers()])
            energy -= reference_energy
        else:
            reference_energy = 0

        if 'forces' in ats.arrays.keys():
            forces = ats.get_forces()
        else:
            forces = np.zeros_like(ats.get_positions())
        return Data(
            edge_index=torch.tensor(edge_index, dtype=torch.long),
            node_attrs=one_hot.type(torch.double),
            positions=torch.Tensor(ats.get_positions()).type(torch.float32),
            z=torch.Tensor(ats.get_atomic_numbers()).type(torch.float32),
            shifts=torch.Tensor(shifts).type(torch.float32),
            unit_shifts=torch.Tensor(unit_shifts).type(torch.float32),
            cell=torch.Tensor(np.array(ats.get_cell()[np.newaxis, :])).type(torch.float32),
            forces=torch.Tensor(forces).type(torch.float32),
            energy=torch.Tensor([energy]).type(torch.float32),
            reference_energy=torch.Tensor([reference_energy]).type(torch.float32),
            n_atoms=torch.Tensor([len(ats)]).type(torch.int),
            pbc=torch.Tensor(np.array(ats.get_pbc()[np.newaxis, :])).type(torch.float32),
        )

    def atoms_to_data_list(self, atoms: list[Atoms]) -> list[Data]:
        """Conver the given list of atoms to torch geometric Data objects."""
        data_list = []
        for i, ats in enumerate(atoms):
            data = self._extract_properties(ats)
            data.idx = torch.IntTensor([i])
            data_list.append(data)
        return data_list

    def atoms_to_batch(self, atoms: list[Atoms]) -> list[Data]:
        """Conver the given list of atoms to torch geometric Data objects."""
        return Batch.from_data_list(self.atoms_to_data_list(atoms))

    def data_list_to_atoms(self, data_list: list[Data]) -> list[Atoms]:
        """Return a list of atoms from the given data list or batch."""
        atoms = []
        # Add back energies from statistics files
        for data in data_list:
            positions = data.positions.cpu().detach().numpy()
            z = data.z.cpu().detach().numpy()
            cell = data.cell.cpu().detach().numpy()[0]
            pbc = data.pbc.cpu().detach().numpy()[0]
            ats = Atoms(
                positions=positions,
                cell=cell,
                pbc=pbc,
            )
            ats.set_atomic_numbers(z)
            calc = SinglePointCalculator(
                atoms=ats,
                energy=data.energy.cpu().detach().numpy()[0],
                forces=data.forces.cpu().detach().numpy(),
            )
            ats.set_calculator(calc)
            atoms.append(ats)
        return atoms

    def batch_to_atoms(self, batch: Batch) -> list[Atoms]:
        """Return a list of atoms from the given data list or batch."""
        return self.data_list_to_atoms(batch.to_data_list())

    def compute_neighbors(self, batch: Batch) -> Batch:
        """Compute the neigbhors of the given batch."""
        batch.to("cpu")
        atoms = self.batch_to_atoms(batch)
        return self.atoms_to_batch(atoms)
